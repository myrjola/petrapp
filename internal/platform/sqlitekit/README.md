...
- **Any concrete `schema.sql` / `fixtures.sql` / `embed.go`.** Those belong to
  ...
  `internal/petra/repository/` (see its
  [README](../../petra/repository/README.md)); `cmd/example` has its own.
  sqlitekit embeds no DDL.
- **Schema-design guidance** (STRICT-mode patterns, FK conventions, the
  premigration escape hatch, schema-evolution process across
  domain/repository/service). That guidance lives with the product schema
  in `internal/petra/repository/README.md`.
...
- **`Premigration`** — optional `func(context.Context, *Database) error` run
  ...
  *structure* only, never data, so it cannot rename a column or populate a new
  ...
  premigration escape hatch in `internal/petra/repository/README.md`.
