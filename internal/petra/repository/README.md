...
transactional boundary at week scope. The rationale is recorded in
...
`internal/platform/sqlitekit` (see its [README](../../platform/sqlitekit/README.md)).
...
database). For a worked example, see `PreMigrateExerciseSlots` in git history
(removed in `f51c3f9` after its production run). Reserve the copy
