...
// Test_playwright_rest_chip verifies the rest-timer chip renders after completing a set,
// displays an M:SS countdown, and ticks down in real time. The server-side render is
// covered by handler-exerciseset_test.go; this test pins the JS interval behaviour in
// a real browser.
func Test_playwright_rest_chip(t *testing.T) {
	if testing.Short() {
	...
	if err = page.WaitForURL(fmt.Sprintf("%s/schedule", serverURL)); err != nil {
		t.Fatalf("expect redirect to /schedule: %v", err)
	}

	// Schedule today (and tomorrow as a midnight-boundary guard).
	testStart := time.Now()
	for _, day := range []string{testStart.Weekday().String(), testStart.AddDate(0, 0, 1).Weekday().String()} {
		if _, err = page.GetByLabel(day).SelectOption(playwright.SelectOptionValues{
			Labels: &[]string{"1 hour"},
		}); err != nil {
			t.Fatalf("select %s duration: %v", day, err)
	...
	if err = page.WaitForURL(fmt.Sprintf("%s/", serverURL)); err != nil {
		t.Fatalf("expect redirect to /: %v", err)
	}
	...
	// Pick the first exercise and complete its warmup.
	firstExercise := page.Locator("a[data-workout-exercise-id]").First()
	if err = firstExercise.Click(); err != nil {
		t.Fatalf("click first exercise: %v", err)
	}
	exerciseURLPattern := regexp.MustCompile(fmt.Sprintf(`/workouts/%s/exercises/\d+$`, today))
	if err = page.WaitForURL(exerciseURLPattern); err != nil {
		t.Fatalf("expect navigation to exercise page: %v", err)
	...
	// Complete the first set. Weighted exercises auto-submit via a signal button;
	// bodyweight exercises require filling reps then clicking "Complete set".
	currentSet := page.GetByRole("group", playwright.PageGetByRoleOptions{Name: "Current set"})
	completedSets := page.Locator(".exercise-set.completed")
	couldDoMoreBtn := currentSet.GetByRole("button",
		playwright.LocatorGetByRoleOptions{Name: "Could have done more reps"})
	...
	if btnCount > 0 {
		if err = couldDoMoreBtn.Click(); err != nil {
			t.Fatalf("click weighted signal: %v", err)
		}
	} else {
		repsInput := currentSet.GetByRole("textbox", playwright.LocatorGetByRoleOptions{Name: "Reps"})
		if err = repsInput.Fill("10"); err != nil {
	...
	// Wait for the post-submit DOM to render the just-completed set — the same
	// pattern used by Test_playwright_smoketest. WaitForURL is unreliable since
	// the submit redirects back to the same DETAIL URL (auto-replaced by the
	// client), so the URL never changes.
	if err = completedSets.Nth(0).WaitFor(); err != nil {
		t.Fatalf("expect set 1 to render as completed: %v", err)
	}

	// The rest chip is rendered server-side in the same response, so it should
	// be present alongside the completed set.
	chip := page.Locator(".rest-chip[data-rest-end-at-ms]")
	if err = chip.WaitFor(); err != nil {
		t.Fatalf("rest chip not visible: %v", err)
	}

	// Verify time text matches M:SS format (the chip starts at "—:—" but the
	// inline tick() call replaces it before paint).
	timeEl := chip.Locator("[data-rest-time]")
	timeText, err := timeEl.TextContent()
	if err != nil {
		t.Fatalf("read chip time text: %v", err)
	}
	matched, err := regexp.MatchString(`^\d{1,2}:\d{2}$`, timeText)
	if err != nil || !matched {
		t.Errorf("rest chip text %q does not match M:SS", timeText)
	}

	// Wait long enough to guarantee the displayed M:SS value rolls over to the
	// next-lower second. The tick() function uses Math.ceil(remaining / 1000),
	// so the displayed second only decrements when the remaining time crosses
	// a full-second boundary. Between two reads we therefore need to wait
	// strictly more than one whole second — a 700ms wait is enough to fire the
	// 250ms setInterval but not enough to guarantee the seconds digit advances.
	// 1200ms covers worst-case alignment plus browser scheduling jitter.
	time.Sleep(1200 * time.Millisecond)
	timeText2, err := timeEl.TextContent()
	if err != nil {
		t.Fatalf("read chip time text after tick: %v", err)
	}
	if timeText2 == timeText {
		t.Errorf("rest chip text did not change after 1200ms: still %q", timeText2)
	}
}
