...
// Test_ExerciseSet_RestChipPersistsAfterElapsed verifies the chip keeps
// rendering after its deadline has passed, so a user who rotated through
// other exercises (power sets) and returned later still sees the slot's
// rest state. The on-screen JS turns elapsed deadlines into "Ready" on the
// client; the server-side job is just to keep the chip on the page.
func Test_ExerciseSet_RestChipPersistsAfterElapsed(t *testing.T) {
	t.Parallel()
	...
	if doc, err = client.SubmitForm(ctx, doc, "/preferences/schedule",
		map[string]string{time.Now().Weekday().String(): "60"}); err != nil {
		t.Fatalf("submit preferences: %v", err)
	}
	...
	// Patch warmup_completed_at to one hour ago to simulate a long
	...
	if doc, err = client.GetDoc(ctx, slotURL); err != nil {
		t.Fatalf("re-fetch exercise: %v", err)
	}
	chip := doc.Find(".exercise-set.active .rest-chip[data-rest-end-at-ms]")
	if chip.Length() == 0 {
		t.Fatal("expected rest-chip to persist on the active card after rest elapsed")
	}
	endAtStr, exists := chip.Attr("data-rest-end-at-ms")
	if !exists {
		t.Fatal("rest chip missing data-rest-end-at-ms attribute")
	...
	if endAtMs >= time.Now().UnixMilli() {
		t.Errorf("expected data-rest-end-at-ms in the past, got %d (now %d)",
...
func TestExerciseSetGET_DeloadHidesSignalButtons(t *testing.T) {
	...
	// Set today as a workout day via the schedule form.
	formData := map[string]string{time.Now().Weekday().String(): "60"}
	if doc, err = client.GetDoc(ctx, "/preferences"); err != nil {
		t.Fatalf("get preferences: %v", err)
	}
	...
	// Patch the preferences row to enable deload with a 4-week mesocycle whose
	// anchor is 3 weeks before this Monday — making this week index 3 (= length-1),
	// which is the deload week.
	now := time.Now().UTC()
	...
	// Anchor 3 weeks earlier → this week = week index 3 in a 4-week block = deload.
	anchor := thisMonday.AddDate(0, 0, -21)
	db := server.DB()
	if _, err = db.ExecContext(ctx, `
		UPDATE workout_preferences
		SET deload_enabled = 1,
		    mesocycle_length = 4,
		    mesocycle_anchor = ?
		WHERE user_id = (SELECT id FROM users ORDER BY id DESC LIMIT 1)`,
		anchor.Format("2006-01-02")); err != nil {
		t.Fatalf("patch deload preferences: %v", err)
	}

	// Delete any sessions that were generated by the preferences form submission (which
	// did not know about deload yet) so that StartSession regenerates the week with the
	// deload flag set correctly.
	sunday := thisMonday.AddDate(0, 0, 6)
	if _, err = db.ExecContext(ctx, `
		DELETE FROM workout_sessions
		WHERE workout_date >= ? AND workout_date <= ?`,
		thisMonday.Format("2006-01-02"), sunday.Format("2006-01-02")); err != nil {
		t.Fatalf("delete pre-existing sessions: %v", err)
	}

	// Start the workout for today — this triggers plan generation with IsDeload=true.
	today2 := time.Now().Format("2006-01-02")
	if doc, err = client.SubmitForm(ctx, doc, "/workouts/"+today2+"/start", nil); err != nil {
		t.Fatalf("start workout: %v", err)
	}

	// Pick the first exercise slot URL from the workout page.
	var slotURL string
	doc.Find("a.exercise").EachWithBreak(func(_ int, s *goquery.Selection) bool {
		if href, exists := s.Attr("href"); exists {
			slotURL = href
	...
	// Fetch the exercise-set page.
	if doc, err = client.GetDoc(ctx, slotURL); err != nil {
		t.Fatalf("get exercise set page: %v", err)
	}

	// The deload banner must appear before warmup is completed.
	if doc.Find(".deload-banner").Length() == 0 {
		t.Error("deload banner not rendered on deload session")
	}
	...
	warmupAction, exists := warmupForm.Attr("action")
	if !exists {
		t.Fatal("warmup form has no action attribute")
	}
	if doc, err = client.SubmitForm(ctx, doc, warmupAction, nil); err != nil {
		t.Fatalf("submit warmup: %v", err)
	}

	// After warmup completion the set form must be active.
